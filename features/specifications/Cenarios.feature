...
@Cenario2
Scenario: 02 - Send a message to the same destination number as scenario 1
    Given the address of the api
...
@Cenario4
Scenario: 04 - Send a message to the recipient exceeding the character limit
    Given the address of the api
    When i enter the message exceeding limit of character
    Then i should see error because the limit of character was exceeded

@Cenario5
Scenario: 05 - Send a message without the message field, only recipient number.
    Given the address of the api
...
@Cenario6
Scenario: 06 - Send a message exceeding the limit of the recipient number field
    Given the address of the api
    When I enter the recipient's number exceeding the allowed limit
    Then I should see error because the limit was exceded

@Cenario7
Scenario: 07 - Send a message with a field number less than 11 digits
    Given the address of the api
